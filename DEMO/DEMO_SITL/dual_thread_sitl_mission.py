...
class DualThreadSITLMission:
    ...
    def __init__(self, config=None):
        ...
        # 线程通信
        self.detection_queue = queue.Queue(maxsize=self.config['detection_queue_size'])  # 主线程->副线程
        self.result_queue = queue.Queue(maxsize=self.config['result_queue_size'])     # 副线程->主线程
        ...
        self.processing_display = {
            'latest_results': {},  # 最新处理结果
            'processing_status': {},  # 处理状态跟踪
            'show_processing_window': True,  # 是否显示副线程处理窗口
        }
    ...
    def _default_config(self):
        ...
        return {
            ...
            'queue_wait_timeout': 5.0,    # 队列等待超时时间(秒)
    ...
    def _processing_loop(self):
        ...
        while self.running:
            try:
                # 从队列获取检测数据包
                package = self.detection_queue.get(timeout=self.config['queue_wait_timeout'])
                if package is None:  # 结束信号
                    break
                
                # 更新处理状态
                self.target_processing_status[package.target_id] = "处理中..."
                
                # 处理图像转正
                print(f"🔄 开始处理目标 {package.target_id}")
                ...
                if correction_info['success']:
                    self.target_processing_status[package.target_id] = "转正完成"
                else:
                    self.target_processing_status[package.target_id] = "转正失败"
                ...
                try:
                    ...
                    if ocr_results:
                        ...
                        ocr_success = True
                        self.target_processing_status[package.target_id] = "OCR完成"
                    else:
                        self.target_processing_status[package.target_id] = "OCR无结果"
                except Exception as e:
                    self.target_processing_status[package.target_id] = "OCR失败"
                ...
                # 更新处理状态
                self.target_processing_status[package.target_id] = "处理完成"
                ...
                display_result = {
                    ...
                    'original_crop': package.crop_image,
                    'ocr_text': ocr_text,
                    'ocr_success': ocr_success,
                    'processing_time': time.time() - package.timestamp
                }
                
                # 更新最新处理结果
                self.processing_display['latest_results'][package.target_id] = display_result
                
                # 将结果放入结果队列（用于实时显示）
                try:
                    self.result_queue.put_nowait(display_result)
                except queue.Full:
                    pass  # 队列满时忽略
                
                print(f"✅ 目标 {package.target_id} 处理完成: 数字={detected_number}, GPS=({target_lat:.6f}, {target_lon:.6f})")
                ...
                if 'package' in locals():
                    self.target_processing_status[package.target_id] = f"错误: {str(e)}"
                continue
    ...
    def run_video_mission(self, video_source):
        ...
        # 设置副线程处理展示窗口
        if self.processing_display['show_processing_window']:
            cv2.namedWindow("副线程处理展示", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("副线程处理展示", 800, 600)
        ...
        try:
            while self.running:
                ...
                # 处理副线程结果并在主窗口显示
                processed_frame = self._draw_processing_results(processed_frame)
                ...
                if key == ord('q'):
                    ...
                    self.processing_display['show_processing_window'] = not self.processing_display['show_processing_window']
                    if not self.processing_display['show_processing_window']:
                        cv2.destroyWindow("副线程处理展示")
                    else:
                        cv2.namedWindow("副线程处理展示", cv2.WINDOW_NORMAL)
                        cv2.resizeWindow("副线程处理展示", 800, 600)
    ...
    def _main_thread_process(self, frame):
        ...
        for i, det in enumerate(detections[:max_targets]):
            ...
            try:
                ...
                # 发送到副线程处理队列 - 确保不跳过任何目标
                queue_put_success = False
                queue_wait_time = 0
                max_wait_time = self.config['queue_wait_timeout']  # 使用配置中的最大等待时间
                ...
                while not queue_put_success and queue_wait_time < max_wait_time:
                    try:
                        ...
                        if queue_wait_time > 0:
                            print(f"✅ 目标 {target_id} 成功加入处理队列 (等待 {queue_wait_time:.1f}s)")
                        
                    except queue.Full:
                        # 队列满时等待一小段时间后重试
                        if queue_wait_time == 0:
                            print(f"⏳ 检测队列已满，正在等待空间处理目标 {target_id}...")
                            self.target_processing_status[target_id] = "等待队列空间..."
                        
                        time.sleep(0.1)  # 等待100ms
                        queue_wait_time += 0.1
                        
                        # 更新处理状态显示等待时间
                        if queue_wait_time > 1.0:  # 超过1秒才显示等待时间
                            self.target_processing_status[target_id] = f"等待队列 {queue_wait_time:.1f}s"
                
                # 如果超过最大等待时间仍无法加入队列，记录警告但继续处理
                if not queue_put_success:
                    print(f"⚠️ 目标 {target_id} 等待队列超时 ({max_wait_time}s)，可能系统负载过高")
                    self.target_processing_status[target_id] = "队列等待超时"
                    
                    # 即使队列满也要保存原始检测数据
                    # 这样至少保证数据完整性
                else:
                    # 成功加入队列，更新状态
                    self.target_processing_status[target_id] = "已加入处理队列"
    ...
    def _draw_main_detection(self, frame, x1, y1, x2, y2, target_id, confidence):
        ...
        # 绘制处理状态
        status = self.target_processing_status.get(target_id, "等待处理...")
    ...
    def _draw_processing_results(self, frame):
        """在主窗口绘制副线程处理结果"""
        # 处理副线程结果队列
        processed_count = 0
        while not self.result_queue.empty() and processed_count < 10:  # 限制每帧处理数量
            try:
                result = self.result_queue.get_nowait()
                self._draw_single_processing_result(frame, result)
                processed_count += 1
            except queue.Empty:
                break
    ...
    def _draw_single_processing_result(self, frame, result):
        """绘制单个处理结果"""
        target_info = result['target_info']
        
        # 在右下角显示最新处理结果
        result_y_start = frame.shape[0] - 150
        result_x_start = frame.shape[1] - 300
        
        # 绘制结果背景
        cv2.rectangle(frame, (result_x_start - 10, result_y_start - 10), 
        ...
        # 显示处理结果信息
        result_lines = [
            f"最新处理: {target_info.target_id}",
            f"识别数字: {target_info.detected_number}",
            f"置信度: {target_info.confidence:.3f}",
            f"GPS: {target_info.latitude:.6f}",
            f"     {target_info.longitude:.6f}",
            f"处理时间: {result['processing_time']:.2f}s"
        ]
        
        for i, line in enumerate(result_lines):
            color = (0, 255, 0) if result['correction_info']['success'] else (0, 165, 255)
            cv2.putText(frame, line, (result_x_start, result_y_start + i * 20), 
        ...
        if result['corrected_image'] is not None:
            try:
                # 缩放图像到合适大小
                small_img = cv2.resize(result['corrected_image'], (60, 60))
                # 放置在结果信息旁边
                img_x = result_x_start - 70
                img_y = result_y_start
                frame[img_y:img_y+60, img_x:img_x+60] = small_img
                
                # 绘制小图像边框
                cv2.rectangle(frame, (img_x, img_y), (img_x+60, img_y+60), (0, 255, 255), 1)
                cv2.putText(frame, "转正后", (img_x, img_y-5), 
    ...
    def _show_processing_window(self):
        ...
        # 绘制标题
        cv2.putText(canvas, "副线程处理实时展示", (20, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        
        # 绘制统计信息
        stats_lines = [
            f"处理队列: {self.detection_queue.qsize()}/{self.detection_queue.maxsize}",
            ...
            f"成功率: {self.processing_thread_stats['correction_success']/max(1,self.processing_thread_stats['total_processed'])*100:.1f}%"
        ]
        
        for i, line in enumerate(stats_lines):
            cv2.putText(canvas, line, (20, 70 + i * 25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
        
        # 显示最近的处理结果
        recent_results = list(self.processing_display['latest_results'].values())[-6:]  # 最近6个结果
        
        for i, result in enumerate(recent_results):
            self._draw_processing_detail(canvas, result, i)
        
        # 显示处理状态
        y_offset = 200
        cv2.putText(canvas, "处理状态:", (20, y_offset), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        
        status_count = 0
        for target_id, status in list(self.target_processing_status.items())[-10:]:  # 最近10个状态
            if status_count >= 10:
                break
            color = self._get_status_color(status)
            status_text = f"{target_id}: {status}"
            cv2.putText(canvas, status_text, (20, y_offset + 30 + status_count * 20), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
            status_count += 1
        
        cv2.imshow("副线程处理展示", canvas)
    
    def _draw_processing_detail(self, canvas, result, index):
        """绘制单个处理详情"""
        x_offset = 20 + (index % 3) * 250
        y_offset = 450 + (index // 3) * 120
        
        # 绘制边框
        cv2.rectangle(canvas, (x_offset, y_offset), (x_offset + 240, y_offset + 110), (100, 100, 100), 1)
        
        # 显示转正前后对比
        if result['original_crop'] is not None and result['corrected_image'] is not None:
            try:
                # 原始图像
                orig_small = cv2.resize(result['original_crop'], (50, 50))
                canvas[y_offset+5:y_offset+55, x_offset+5:x_offset+55] = orig_small
                cv2.putText(canvas, "原始", (x_offset+5, y_offset), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1)
                
                # 转正后图像
                corr_small = cv2.resize(result['corrected_image'], (50, 50))
                canvas[y_offset+5:y_offset+55, x_offset+60:x_offset+110] = corr_small
                cv2.putText(canvas, "转正", (x_offset+60, y_offset), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0), 1)
                
                # 显示箭头
                cv2.arrowedLine(canvas, (x_offset+55, y_offset+30), (x_offset+60, y_offset+30), 
                               (0, 255, 255), 2)
            except Exception as e:
                pass
        ...
        target_info = result['target_info']
        info_lines = [
            f"ID: {target_info.target_id}",
            f"数字: {target_info.detected_number}",
            f"OCR: {result['ocr_text'][:10]}..." if len(result['ocr_text']) > 10 else f"OCR: {result['ocr_text']}",
            f"时间: {result['processing_time']:.2f}s"
        ]
        
        for i, line in enumerate(info_lines):
            color = (0, 255, 0) if result['correction_info']['success'] else (0, 165, 255)
            cv2.putText(canvas, line, (x_offset + 120, y_offset + 15 + i * 15), 
